...
// Operations
	DWORD AddThread(LPTSTR lpstrCmdLine, int nCmdShow)
	{
		...
		{
			::MessageBox(NULL, _T("ERROR: Cannot create ANY MORE threads!!!"), _T("WebView2"), MB_OK);
			return 0;
		}
		...
		if(hThread == NULL)
		{
			::MessageBox(NULL, _T("ERROR: Cannot create thread!!!"), _T("WebView2"), MB_OK);
			return 0;
		}
	...
	int Run(LPTSTR lpstrCmdLine, int nCmdShow)
	{
		...
		while(m_dwCount > 0)
		{
			...
			if(dwRet == 0xFFFFFFFF)
			{
				::MessageBox(NULL, _T("ERROR: Wait for multiple objects failed!!!"), _T("WebView2"), MB_OK);
			}
