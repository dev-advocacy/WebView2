...
namespace WebView2
{
    ...
    void CompositionHost::Initialize(HWND hwnd, wil::com_ptr<ICoreWebView2Controller> controller, wil::com_ptr<ICoreWebView2CompositionController> compositionController)
    {
        if (controller && compositionController)
        {
            m_controller = controller;
            m_compositionController = compositionController;
            GetClientRect(hwnd, &m_appBounds);
    ...
    void CompositionHost::CreateDesktopWindowTarget(HWND window)
    {
        ...
        check_hresult(interop->CreateDesktopWindowTarget(window, false, reinterpret_cast<abi::IDesktopWindowTarget**>(put_abi(m_target))));
    }
    ...
    HRESULT CompositionHost::CreateWebViewVisual()
    {
        if (m_controller && m_rootVisual)
        {
            ...
            RETURN_IF_FAILED(m_compositionController->put_RootVisualTarget(m_webViewVisual.as<IUnknown>().get()));
        }
        return S_OK;
    }
    ...
    HRESULT CompositionHost::UpdateVisual(POINT point, UINT message, WPARAM wParam)
    {
        ...
        else if (selectedVisual == m_webViewVisual)
        {
            ...
            RETURN_IF_FAILED(m_compositionController->SendMouseInput(static_cast<COREWEBVIEW2_MOUSE_EVENT_KIND>(message), static_cast<COREWEBVIEW2_MOUSE_EVENT_VIRTUAL_KEYS>(GET_KEYSTATE_WPARAM(wParam)), 0, point));
        }
        return S_OK;
    }
    ...
    {
        uint8_t random = static_cast<uint8_t> ((double)(double)(rand() % 255));
        return ColorHelper::FromArgb(255, 50, 120, random);
    }

    void CompositionHost::SetBounds(CRect bounds)
    {
        m_appBounds = bounds;
